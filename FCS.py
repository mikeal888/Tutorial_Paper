...
def jump_ops(m_ops, method):
    if method == "PD":
        ...
        raise NameError(
            "Check method. It should be 'PD' or 'Homodyne', not: {} ".format(method)
        )
...
def TwoTimeCorrelationSS(H, t, c_ops, rho, m_ops, mu, method="PD"):
    ...
    Ft = (
        np.array(
            [
                np.real((Ivec.trans() * La * (L * ti).expm() * La * rhovec)[0, 0])
                for ti in t
            ]
        )
        - np.sum(mu * Ja) ** 2
...
def FCSPowerSpectrumLinear(H, c_ops, rho, omega, m_ops, mu, method="PD"):
    ...
    if N < 10:
        ...
        for i, omegai in enumerate(omega):
            v1 = Qobj(
                np.linalg.lstsq(L - 1j * omegai, yf, rcond=None)[0], dims=rhovec.dims
            )
            v2 = Qobj(
                np.linalg.lstsq(L + 1j * omegai, yf, rcond=None)[0], dims=rhovec.dims
            )
